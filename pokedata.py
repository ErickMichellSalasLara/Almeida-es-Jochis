...
MOVIMIENTOS = {
    ...
    #Multigolpes usando el mismo daño y usando recursividad (Lo limittaremos a 5 golpes)
    "Sape Recursivo": Movement("Sape Recursivo", "TICS", 20, 95),
    ...
    #Daño exponencial, osease, hará el x2 de daño en cada  recursion (Lo limitaremos a 3 golpes)
    "Ducha de Agua Fría": Movement("Ducha de Agua Fría", "DUCHA", 10, 90),
    ...
    #Daño incremental (Cada golpe hará más daño que el anterior)
    "Deuda Coppel": Movement("Deuda Coppel", "JORNADA_LABORAL", 10, 90),
...
class STATS_BASE:
    ...
    # Aqui haremos los ataques usando recursividad
    # Sape Recursivo (Suma daño fijo)
    def recursivo_multigolpe(self, daño_base, golpes_restantes):
        # CASO BASE
        if golpes_restantes <= 0:
            return 0
        # CASO RECURSIVO
        return daño_base + self.recursivo_multigolpe(daño_base, golpes_restantes - 1)
    
    # Deuda Coppel (El daño aumenta un 50% en cada golpe)
    def recursivo_incremental(self, daño_base, golpes_restantes, multiplicador = 1.0):
        # CASO BASE
        if golpes_restantes <= 0:
            return 0
        # CASO RECURSIVO
        daño_actual = daño_base * multiplicador
        return daño_actual + self.recursivo_incremental(daño_base, golpes_restantes - 1, multiplicador + 0.5)
    
    # Ducha de Agua Fría (Daño que se duplica en cada iteración)
    def recursivo_exponencial(self, daño_base, repeticiones):
        # CASO BASE
        if repeticiones <= 0:
            return 0
        # CASO RECURSIVO
        return daño_base + self.recursivo_exponencial(daño_base * 2, repeticiones - 1)
    ...
    def calcular_daño(self, atacante, defensor, movimiento):
        ...
        daño_un_golpe = (ataque / defensa) * potencia * efectividad
        daño_base = max(1, int(daño_un_golpe))

        #Se llamara a usar la funcion recursiva si es que se usa el movimiento
        if movimiento.nombre == "Sape Recursivo":
            return self.recursivo_multigolpe(daño_base, 5)
            
        elif movimiento.nombre == "Ducha de Agua Fría":
            return self.recursivo_exponencial(daño_base, 3)
            
        elif movimiento.nombre == "Deuda Coppel":
            return self.recursivo_incremental(daño_base, 4)
            
        else:
            return daño_base # Si es normal, solo devuelve el daño base
...
class Andrew(STATS_BASE):
    def __init__(self):
        ...
        self.movimientos = [MOVIMIENTOS["Ducha de Agua Fría"], MOVIMIENTOS["Baño"], MOVIMIENTOS["Es solo un amigo"], MOVIMIENTOS["Actitud"]]
        #--------- Aqui se completa la clase
#Ya tiene imagen
...
class Osmar(STATS_BASE):
    def __init__(self):
        ...
        self.movimientos = [MOVIMIENTOS["Cálculo Mental"], MOVIMIENTOS["Integral Doble"], MOVIMIENTOS["Sape Recursivo"], MOVIMIENTOS["Brillo Propio"]]
        #--------- Aqui se completa la clase
#Ya tiene imagen -
...
class Almeida(STATS_BASE):
    def __init__(self):
        ...
        # Ahora agregamos los movimientos que CachimbaBoy tendrá
        self.movimientos = [MOVIMIENTOS["Deuda Coppel"], MOVIMIENTOS["Trabajo Duro"], MOVIMIENTOS["Encanto Brillante"], MOVIMIENTOS["Aullido"]]
        #--------- Aqui se completa la clase