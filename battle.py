...
class BattleManager:
    ...
    def aplicar_accion(self, atacante, defensor, movimiento):
        ...
        #Movimiento de daño (potencia > 0)
            ...
            #Textos especificos para cuando son los recursivos.
            if movimiento.nombre == "Sape Recursivo":
                self.log.append(f"¡Sapearon 5 veces a {defensor.nombre}!")
            elif movimiento.nombre == "Ducha de Agua Fría":
                self.log.append(f"¡{defensor.nombre} lo bañaron con agua fría!")
            elif movimiento.nombre == "Deuda Coppel":
                self.log.append(f"¡A {defensor.nombre} le cobraron los interes acumulados!")
